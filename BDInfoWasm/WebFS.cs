...
public class WebFS
{
    public class WebDirectoryInfo : IDirectoryInfo
    {
        ...
        public WebDirectoryInfo(WebFile[] fileTree, string path)
        {
            _fileTree = fileTree;
            // Use a List in a stack-based approach to scan _fileTree iteratively for a node that has a path matching the given path
            var stack = new Stack<WebFile>(_fileTree);
            while (stack.Count > 0)
            {
                var node = stack.Pop();
        ...
        public static IDirectoryInfo FromDirectoryName(WebFile[] fileTree, string path)
        {
            return new WebDirectoryInfo(fileTree, path);
        }
    }
